from tkinter import*
root = Tk()
root.title("Vedansh Ruhela's first own Calculator")
root.minsize(width=364, height=523)
root.maxsize(width=364, height=523)
icon = PhotoImage(file="calculator-icon-8175.png") 
...
def button(source, side, text, command=None):
    storeObj = Button(source, bg="yellow", fg="gray5", text=text, command=command)
    storeObj.pack(side=side, expand=YES, fill=BOTH)
    return storeObj
...
class app(Frame):
    def __init__(self):
        ...
        Entry(self, relief=RIDGE,
                textvariable=display,justify='right',bd=26,fg="white",bg="blue").pack(side=TOP, expand=YES,
                        fill=BOTH)
